function Jsequencing(options){
  ...
  //item内容
  function itemhtml(imgsrc,imgtitle){
  ...
  for(var j=0;j<opts.imgsrcarr.length;j++){
    ...
    html+='<div class="item" id="'+opts.listid+'_item'+j+'" item="'+j+'">'+itemhtml(vimgsrc,vimgtitle)+'</div>';
  }
  itemlist.html(html);
  ...
  //绘制/移动
  var draw = function(dom,col,few,slidetime){
    dom.css({
  ...
  //计算位置
  var computat = function(index,domid,slidetime){
    var item = $("#"+domid);
    item.attr({
      "item":index,
    });
  ...
  //调用绘制
  var redraw = function(strati,ilen,slidetime){
    for(var j=strati;j<strati+ilen;j++){
      computat(j,opts.listid+"_item"+itemidarr[j],slidetime);
    }
...
function distory(val){
  $('#'+val).empty();
}
// 返回在vue模板中的调用接口
export default {
  Jsequencing: function (val) {
    return Jsequencing(val)
  },
  distory:function(val){
    return distory(val)
  }
}
