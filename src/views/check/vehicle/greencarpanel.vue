...
export default {
  ...
  methods: {
    ...mapActions({
      commitInspectionManagementVehicle: "commitInspectionManagementVehicle"
    }),
    // 设置审核意见
    setLabel(e) {
    ...
    parseTimeLine(data) {
      ...
      for (let j = 0; j < data.verifySuggestList.length; j++) {
        arr.push({
          ...
          // 审核状态名称
          verifyStatusName: data.verifySuggestList[j].verifyStatusName,
          // 审核意见
          isGcTrunckName: data.verifySuggestList[j].verifySuggestSelect,
          // 意见描述
    ...
    summit(e) {
      if (e) {
        ...
        this.commitInspectionManagementVehicle(this.verifyData).then(res => {
          if (res === "sucess") {
            this.init();
          }
        });
