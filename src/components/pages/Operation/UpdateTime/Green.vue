  ...
  export default {
    ...
    computed:{
      GreentableData: function(){
        ...
        let _GreentableData=[];
        for (let i = 0; i < this.GreenTime.length; i++) {
          let state1 = this.Contrast1(this.GreenTime[i].maxTollTime);
          // console.log(state1);
          _GreentableData.push({
            // late1: state1[0],
            lateTime1: state1 ,
            id1: this.GreenTime[i].orgId,
            name1: this.GreenTime[i].orgName,
            time1: this.GreenTime[i].maxTollTime,
          })
        }
