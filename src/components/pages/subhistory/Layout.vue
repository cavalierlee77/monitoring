...
<script>
  ...
  export default {
    ...
    components: {
      ...
      GreenQb: () =>
        import(
          /* webpackChunkName: "subhistory" */ "./_template/_querybox/GreenQuerybox.vue"
        ),
      TruckQb: () =>
        import(
          /* webpackChunkName: "subhistory" */ "./_template/_querybox/TruckQuerybox.vue"
        ),
      CombineHarvQb: () =>
        import(
          /* webpackChunkName: "subhistory" */ "./_template/_querybox/CombineHarvQuerybox.vue"
        ),
      MilitaryQb: () =>
        import(
          /* webpackChunkName: "subhistory" */ "./_template/_querybox/MilitaryQuerybox.vue"
        ),
      DisasterQb: () =>
        import(
          /* webpackChunkName: "subhistory" */ "./_template/_querybox/DisasterQuerybox.vue"
        ),
      CheckedDetails: () =>
        import(
          /* webpackChunkName: "subhistory" */ "./_template/_details/Details"
        )
    },
    computed: {
      ...mapState({
        cartabs: state => state.subhistory.cartabs,
        height: state => state.common.layoutHgt - 114,
        historydata: state => state.subhistory.historyData,
        nowTab: state => state.subhistory.nowTab,
        tabType: state =>
          state.subhistory.nowTab
            ? state.subhistory.hisRelevance[state.subhistory.nowTab]
            : state.subhistory.hisRelevance.GreenCar,
        HeadList: state =>
          state.subhistory.nowTab
            ? state.subhistory[
                state.subhistory.hisRelevance[state.subhistory.nowTab].header
              ]
            : state.subhistory.GreenCarHeaderList
      }),
      page: function() {
        return this.$store.state.subhistory[this.tabType.page];
      }
    },
    methods: {
      ChangeTabs(val) {
        this.$store.commit("setSubHisNowTab", val);
      ...
      ChangePage(val) {
        this.$store.commit("setSubHisNowPage", val);
      ...
      getHistoryData(val = {}) {
        for (const [k, v] of Object.entries(val)) {
          if (
            ...
            this.$store.commit("setSubHisNowPage", _page);
            break;
          }
        }
        ...
        this.$store.dispatch("getSubHisHistoryData", fm);
      },
