<template>
  <div class="">
    <header>
      <i class="el-icon-info"></i> 查验图片/视频
...
<script>
// 组件中判定URL类型
// import '../../../static/picture/jquery.magnify.css'
// import '../../../static/picture/jquery.magnify.js'
export default {
  name: "",
  props: {
    picture_video_data: {
      ...
      this.url.forEach((item, index) => {
        if (item == val) {
          this.url.splice(
            index,
            1,
            val.substring(0, val.lastIndexOf("?")) + "?" + Math.random()
          );
        }
      });
    },
