...
const state = {
  ...
  postList: [
    { id: "", name: "全部" },
    { id: "videoExaMark", name: "视频巡查员" },
    { id: "chargeMark", name: "收费科" },
    { id: "regulMark", name: "纪检审" },
    { id: "directorMark", name: "副局" }
  ],
  goodsList: [],
  ...
  hisRelevance: {
    GreenCar: {
      ...
      posturl: "subcheck/getSubQueryCheckRecords",
...
// actions
const actions = {
  getSubHisHistoryData({ commit, state }, fmdata) {
    const posturl = state.hisRelevance[state.nowTab].posturl;
    commit("setSubHisHistoryData", []);
    return new Promise((resolve, reject) => {
      api.post(posturl, fmdata, res => {
        if (res.resultCode === "100") {
          commit("setSubHisHistoryData", res.resultData);
          commit("setSubHisTotal", res.page.rows);
          resolve();
        } else {
          reject(new Error());
        }
  ...
  getSubHisStatusList({ commit }) {
    return new Promise((resolve, reject) => {
      api.post(
        ...
        res => {
          if (res.resultCode === "100") {
            commit("setSubHisStatusList", res.resultData);
            resolve();
          } else {
            reject(res);
          }
  ...
  async getSubStationList({ commit }) {
    const subStats = function() {
      return new Promise((resolve, reject) => {
        api.post("comm/getSubStats", {}, res => {
          if (res.resultCode === "100") {
            resolve(res);
          } else {
            reject(res);
    ...
    const branchTolls = function(subOrgId) {
      return new Promise((resolve, reject) => {
        api.post("comm/getBranchTolls", { subStatOrgId: subOrgId }, res => {
          if (res.resultCode === "100") {
            resolve(res);
          } else {
            reject(res);
    ...
    const orgData = await subStats();
    const tollData = await branchTolls(orgData.resultData[0].orgId);
    commit("setSubStationList", tollData.resultData);
  },
  getSubHisGoodsList({ commit }) {
    return new Promise((resolve, reject) => {
      api.post("comm/getGoodList", {}, res => {
        if (res.resultCode === "100") {
          ...
          commit("setSubHisGoodsList", _goodsList);
          resolve();
        } else {
...
// mutations
const mutations = {
  setSubHisHistoryData(state, data) {
    state.historyData = data;
  },
  setSubHisNowTab(state, data) {
    state.nowTab = data;
  },
  setSubHisNowPage(state, data) {
  ...
  setSubHisTotal(state, data) {
    state[state.hisRelevance[state.nowTab].page].Total = data;
  },
  setSubStationList(state, data) {
    state.stationList = data;
  },
  setSubHisStatusList(state, data) {
    state.statusList = data;
  },
  setSubHisGoodsList(state, data) {
